        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="soft-card p-5 border-l-4 border-l-indigo-500 hover:-translate-y-1 transition-all">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-slate-500">veículos ativos</span>
            </div>
            <p className="text-xs text-slate-400 mt-2">{completedCount} concluídos em histórico</p>
          </div>
          ...
          <div className="soft-card p-5 border-l-4 border-l-blue-500 hover:-translate-y-1 transition-all">
            ...
            {topStages.length > 0 ? (
              ...
              <p className="text-xs text-slate-400 italic">Nenhuma etapa em andamento.</p>
            )}
          </div>
          ...
          <div className="soft-card p-5 border-l-4 border-l-red-500 hover:-translate-y-1 transition-all">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-slate-500">Docs rejeitados</span>
            </div>
            {rejectedDocs.length > 0 && (
              <p className="text-[10px] text-red-500 mt-2 line-clamp-1">
                Atenção em: {rejectedDocs.map(p => p.customer_name).join(', ')}
              </p>
            )}
            {rejectedDocs.length === 0 && (
              <p className="text-[10px] text-green-600 mt-2 flex items-center gap-1"><Check className="w-3 h-3"/> Tudo em ordem</p>
            )}
          </div>
            ...
            {atRiskProjects.length > 0 ? (
              <div className="space-y-2">
                {atRiskProjects.slice(0, 3).map(p => {
                  ...
                  return (
                    <Link href={`/projects/${p.id}`} key={p.id} className="flex justify-between items-center text-xs group">
              ...
              <div className="flex flex-col items-start gap-1">
                ...
                <span className="text-xs text-green-600 flex items-center gap-1"><Check className="w-3 h-3"/> Entregas sob controle</span>
              </div>
            )}
          </div>
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-1">
                        ...
                        {isLate && (
                          <span className="flex items-center gap-0.5 text-[10px] font-medium text-orange-600 bg-orange-50 border border-orange-100 px-1.5 py-0.5 rounded">
